...
public class SysEmailService : IDynamicApiController, ITransient
{
    ...
    public SysEmailService(IOptions<EmailOptions> emailOptions, IFluentEmail fluentEmail)
    {
    ...
    public async Task SendEmail([Required] string content, string title = "Admin.NET 系统邮件")
    {
        ...
        //使用FluentEmail.Core发生邮件
        //await _fluentEmail.To(_emailOptions.DefaultToEmail).Subject(title).Body(content, true).SendAsync();
